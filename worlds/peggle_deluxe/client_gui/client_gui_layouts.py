...
class NotConnectedLayout(BoxLayout):
    ...
    def __init__(self, ctx: PeggleDeluxeContext) -> None:
...
class PeggleDeluxeLevelInformationLayout(BoxLayout):
    ctx: PeggleDeluxeContext
    ...
    def update(self) -> None:
        ...
        self.gold_pegs_label.text = (
            f"[b]Gold Pegs[/b]\n"
            f"Retrieved [color=00FA9A]{gold_pegs_obtained}[/color] of "
            f"[color=00FA9A]{self.ctx.game_controller.option_gold_pegs_required}[/color] needed "
            f"([color=888888]{self.ctx.game_controller.option_gold_pegs_total} total[/color])"
        )
        ...
        if game_state is not None:
            if game_state.context == PeggleDeluxeContexts.INVALID:
                ...
                if game_state.current_level is not None and game_state.current_character is not None:
                    ...
                    selected_levels: List[PeggleDeluxeLevels] = self.ctx.game_controller.selected_levels
...
class PeggleDeluxeLevelsLayout(BoxLayout):
    ...
    def __init__(self, ctx: PeggleDeluxeContext) -> None:
        ...
        levels: List[PeggleDeluxeLevels] = self.ctx.game_controller.selected_levels

        if self.ctx.game_controller.selected_goal_level is not None:
            levels.append(self.ctx.game_controller.selected_goal_level)

        level: PeggleDeluxeCharacters
        for level in levels:
...
class PeggleDeluxeContent(ScrollView):
    ...
    layout_level_information: PeggleDeluxeLevelInformationLayout
    layout_masters: PeggleDeluxeMastersLayout
    layout_levels: PeggleDeluxeLevelsLayout
    ...
    def __init__(self, ctx: PeggleDeluxeContext) -> None:
        ...
        self.layout_level_information = PeggleDeluxeLevelInformationLayout(ctx=self.ctx)
        self.layout.add_widget(self.layout_level_information)

        self.layout_masters = PeggleDeluxeMastersLayout(ctx=self.ctx)
        self.layout.add_widget(self.layout_masters)

        self.layout_levels = PeggleDeluxeLevelsLayout(ctx=self.ctx)
...
class PeggleDeluxeTabLayout(BoxLayout):
    ...
    layout_content_peggle_deluxe: PeggleDeluxeContent
    ...
    def update(self) -> None:
        if self.ctx.game_controller.target_scores is None:
            ...
            if hasattr(self, "layout_content_peggle_deluxe"):
                self.layout_content_peggle_deluxe.timer.cancel()
        ...
        if not len(self.layout_content.children):
            self.layout_content_peggle_deluxe = PeggleDeluxeContent(ctx=self.ctx)
            self.layout_content.add_widget(self.layout_content_peggle_deluxe)
