...
class PeggleDeluxeContext(CommonClient.CommonContext):
    ...
    def on_package(self, cmd: str, _args: Any) -> None:
        if cmd == "Connected":
            ...
            self.game_controller.option_gold_pegs_total = _args["slot_data"]["gold_pegs_total"]
            self.game_controller.option_gold_pegs_required = _args["slot_data"]["gold_pegs_required"]
            ...
            self.game_controller.option_useful_item_weights = _args["slot_data"]["useful_item_weights"] = {
                PeggleDeluxeAPUsefulItems(item_name): weight for item_name, weight in _args["slot_data"]["useful_item_weights"].items()
            }

            # Generated Data
            self.game_controller.selected_masters = [
                PeggleDeluxeCharacters(master_name) for master_name in _args["slot_data"]["selected_masters"]
            ]

            self.game_controller.selected_starter_master = PeggleDeluxeCharacters(
            ...
            self.game_controller.target_scores = {
                PeggleDeluxeLevels(level_name): scores for level_name, scores in _args["slot_data"]["target_scores"].items()
            }

            self.game_controller.target_score_ratios = {
                PeggleDeluxeLevels(level_name): ratio for level_name, ratio in _args["slot_data"]["target_score_ratios"].items()
            }
            ...
            # Data Storage
            self.data_storage_key = f"peggle_deluxe_{self.team}_{self.slot}"
                ...
                if not self.game_controller.is_process_running():
                    if not self.game_controller.open_process_handle():
                        if self.can_display_process_not_found_message:
                            CommonClient.logger.info("Looking for Peggle Deluxe process...")
...
def main(*args) -> None:
    Utils.init_logging("PeggleDeluxeClient", exception_logger="Client")

    parser = CommonClient.get_base_parser(description="Peggle Deluxe Client")
    ...
    async def _main(_args):
        ctx: PeggleDeluxeContext = PeggleDeluxeContext(args.connect, args.password)
