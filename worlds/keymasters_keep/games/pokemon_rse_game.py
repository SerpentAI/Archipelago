...
class PokemonRSEGame(Game):
    ...
    def optional_game_constraint_templates(self) -> List[GameObjectiveTemplate]:
        return [
            GameObjectiveTemplate(
                label="Using POKEMON as your lead (not applicable for Battle Factory)",
                data={"POKEMON": (self.wild_pokemon, 1)},
                weight=3
            ),
            GameObjectiveTemplate(
                label="Using RAREPOKEMON as your lead (not applicable for Battle Factory)",
    ...
    def battle_objectives(self) -> List[GameObjectiveTemplate]:
        objectives: List[GameObjectiveTemplate] = [
            ...
            GameObjectiveTemplate(
                label="Without using Fly or items, travel between the following cities, defeating every wild encounter you see: CITY",
                data={"CITY": (self.cities, 2)},
                is_time_consuming=True,
                is_difficult=False,
                weight=35,
